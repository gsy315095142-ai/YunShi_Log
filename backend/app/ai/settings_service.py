...
from app.ai.providers.base import get_default_base_url, list_providers
...
def require_chat_credentials(db: Session, user: User) -> tuple[str, str, str]:
    """返回 (provider, api_key, base_url)，供聊天模块调用。"""
    settings = _get_or_create_settings(db, user)
    ...
    base_url = settings.api_base_url or get_default_base_url(settings.provider)
    return settings.provider, api_key, base_url
